...
export const handlePayment = async ({
  ...
  navigate, user, quantityMap, dispatch, orderId, transactionId,rentalId
}) => {
  try{
    ...
    Toast.show({ content: `Thanh toán thất bại: ${error.message}`, duration: 2000 });
  }
};
export const handlePaymentWithMomo = async ({
  ...
  try {
    const response = await fetch('https://localhost:5001/api/momopayment/create', {
      method: 'POST',
      headers: {
